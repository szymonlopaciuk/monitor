...
from .display import display_clear, display_show, display_sleep
...
@click.option(
    ...
    help=(
        'Delay in seconds before refreshing the display. Zero value '
        'means the display will only be refreshed once, after which '
...
def run(output, flip, transit_stop, delay):
    ...
    try:
        while True:
            im = draw_everything(transit_stop)
            if flip:
            ...
            if delay == 0:
                ...
                logger.info('Waiting %d seconds before refreshing', delay)
                time.sleep(delay)
    finally:
        # If for whatever reason the program exits, make sure the
        # display is put to sleep.
        display_sleep(output)


def draw_everything(transit_stop):
    ...
    section_gap = 40
    # Transportation info
    _, y = draw_text(draw, (10, 10), f'Public Transport ({transit_stop})', font=Font.H1, fill=ColorScheme.BLACK,
    ...
    y += section_gap
    # Hourly forecast
    _, y = draw_text(draw, (10, y), 'Temperature and Precipitation', font=Font.H1, fill=ColorScheme.BLACK,
                     halign='left', valign='top')
    ...
    draw_text(draw, (470, y - char_half), f'{rain_max:.1f} mm', font=Font.BODY, fill=ColorScheme.BLACK, halign='right',
              valign='top')
    draw_text(draw, (470, y + chart_h - char_half), f'0', font=Font.BODY, fill=ColorScheme.BLACK, halign='right',
              valign='top')
    _y = y
    ...
    y = _y + section_gap
    # Daily forecast
    _, y = draw_text(draw, (10, y), 'Weather This Week', font=Font.H1, fill=ColorScheme.BLACK, halign='left',
                     valign='top')
    cell_w = 460 / 7
